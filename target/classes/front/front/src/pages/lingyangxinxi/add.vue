		...
		<el-form
			...
			<el-form-item class="add-item" label="是否绝育" prop="shifoujueyu">
				<el-input v-model="ruleForm.shifoujueyu" 
					placeholder="是否绝育" clearable :disabled=" false  ||ro.shifoujueyu"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="申领时间" prop="shenlingshijian">
				<el-date-picker
					:disabled=" false  ||ro.shenlingshijian"
			...
			<el-form-item class="add-item" label="申领理由" prop="shenlingliyou">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="申领理由"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`lingyangxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`lingyangxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
