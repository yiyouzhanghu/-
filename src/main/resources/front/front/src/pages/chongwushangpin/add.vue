<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="适用宠物" prop="shiyongchongwu">
				<el-input
					type="textarea"
					:rows="8"
					placeholder="适用宠物"
					v-model="ruleForm.shiyongchongwu">
					</el-input>
			</el-form-item>
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`chongwushangpin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
