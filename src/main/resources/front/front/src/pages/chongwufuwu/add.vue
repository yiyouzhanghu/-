<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="人数" prop="columns">
				<el-input v-model.number="ruleForm.columns" 
					placeholder="人数" clearable :disabled=" false  ||ro.columns"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="次数" prop="rows">
				<el-input v-model.number="ruleForm.rows" 
					placeholder="次数" clearable :disabled=" false  ||ro.rows"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="总数" prop="totals">
				<el-input v-model="totals" placeholder="总数" disabled></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="可约时间" prop="opentime">
				<el-input v-model="ruleForm.opentime" 
					placeholder="可约时间" clearable :disabled=" false  ||ro.opentime"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`chongwufuwu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`chongwufuwu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
